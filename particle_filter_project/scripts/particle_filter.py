...
def euler_from_quaternion(quat):
    ...
    Assuming planar robot: roll = pitch = 0.
    """
    x, y, z, w = quat
...
def compute_prob_zero_centered_gaussian(dist, sd):
    """ 
    Takes in distance from zero (dist) and standard deviation (sd) for a Gaussian
    and returns the probability (likelihood) of the observation.
    """
...
class ParticleFilter:
    ...
    def update_particle_weights_with_measurement_model(self, data):
        ...
        # to the earlier examples: consider only a few directions (like forward beam).
